...
1 plot 过程绘曲线图
  ...
  1.5 添加标注 过程xyouts用于再图形窗口内添加标注信息
    ...
    其中，参数x,y用于设置标注的位置；参数string为标准的字符串内容；关键字alignment用于设置标注的对齐方式，默认值0为左对齐，0.5为居中对齐，1为右对齐；关键字color用于设置标准的颜色，关键字charsize用于设置标注的字体大小；
    关键字data、device和normal分别用于设置参数
  ...
  1.7 图形保存为文件
    ...
    device[,/close_file{cgm,metafile,ps}][,filename=filename{cgm,metafile,ps}][,/index_color{metafile,printer}][,/true_color{metafile,printer}]
    [,xsize=width{metafile,printer,ps}][ysize=height{metafile,printer,ps}]
    ...
    其中，参数x0和y0用于设置读取的起始列号和行号，默认值均为0，参数nx和ny用于设置读取的列数和行数；关键字channel用于设置读取的波段，如果设备显示的是24位真彩色图像，而该关键字和true关键字均没有设置的话默认返回RGB3个波段中的最高值；
    关键字order用于设置图像纵坐标从上往下算，不设置的话默认从下往上算；
...
2 plot 函数绘曲线图
    ...
    参数fname用于设置图形输出的文件名，关键字bitmap仅适用于emf，eps和pdf文件格式，用于设置输出为位图格式文件，如果该关键字未设置则默认输出为矢量格式，关键字border用于设置图形周围的边框范围，
    如果该关键字未设置则默认保留图形对象中图形周围的所有空白边框区，如果输出文件为矢量格式那么该关键字无效；关键字height用于设置输出文件的高度，对于图形文件其单位为像素，对于pdf文件其单位为英寸，如果该关键字设置则resolution将被忽略，
    且输出文件的高度将根据图形窗口的长宽比自动计算。关键字resolution用于设置输出图形文件的分辨率(单位为：DPI，默认值为600)，关键字centimeters仅对pdf文件有效，设置其为厘米
    ...
    其中，参数x和y用于设置标注的位置(横坐标和纵坐标值)；参数string位标注的内容；关键字target用于设置在哪一个图形对象中添加标注，如果该关键字未设置则默认在当前图形对象中添加标注；关键字color用于设置标注的颜色；关键字font_size用于设置标注的字体大小，
    默认值为16；关键字alignment用于设置标注的水平对齐方式，默认值0为左对齐，0.5为居中对齐，1为右对齐；关键字vertical_alignment用于设置标注的垂直对齐方式，默认值0为底端对齐，0.5为居中对齐，1为顶端对齐；
    关键字data,device和normal分别设置参数x和y的坐标值为数据坐标，设备坐标和归一化坐标。若都没设置默认为归一化坐标。
    2.8 添加图例
    ...
    其中，关键字target为字符串数组，用于设置添加哪些图形对象的图例，如果该关键字未设置则默认添加当前图形对象的图例；关键字label用于设置图例要素的显示名称；关键字postion为一个2元素的数组[x,y],用于设置图例左上角在窗口中的位置；关键字smaple_width用于设置图例中曲线的宽度，默认为0.15；
    关键字auto_text_color用于设置将图例字体颜色修改为其所关联的曲线颜色，如果该关键字设置的法text_color将被忽略；关键字text_color用于设置图例字体的颜色；关键字font_size用于设置图例字体的大小，默认值为16；关键字color用于设置图例边框的颜色；
    关键字thick用于设置图例边框的线宽，关键字horizontal_alignment用于设置图例的水平对齐方式，0为左对齐，0.5为居中对齐，1为右对齐；关键字vertical_alignment用于设置图例的垂直对齐方式，0为底端对齐，0.5为居中对齐，1为顶端对齐；
    关键字horizontal_spacing和vartical_spacing用于设置图例要素之间的水平间隔和垂直间隔(单位为归一化坐标)
3 柱状图
    过程 bar_plot 用于绘制柱状图
    语法：bar_plot,values[,background=value][,colors=vector][,barnames=string_array][,barwidth=value][,baroffest=value][,barspace=value][,baselines=vector][,title=string]
    [,xtitle=string][,ytitle=string][,/outline][,/overplot][,/rotate]
    其中，参数values为绘图的数据；关键字background用于设置背景的颜色；关键字color用于设置各个柱体的颜色，与参数values元素数目相同；关键字barnames用于设置每个柱体的名称；关键字barwidth用于设置柱体的宽度；
    关键字baroffset用于设置第一个柱体的位置；关键字barspace用于设置柱体之间的间隔；关键字baselines用于设置柱体的纵坐标的起始值；关键字title用于设置图形的标题；关键字xtitle和ytitle分别用于设置x轴和y轴的标题；关键字outline用于设置将各个柱体加上边框；
    关键字overplot用于设置将当前图形叠加到现有图形上面，如果该关键字未设置则打开一个新的窗口绘图；关键字rotate用于设置将图形横过来，即绘制条形图
    
    除了传统的bar_plot过程之外，  IDL 8.0提供了一个新的barplot函数绘制柱状图
    函数barplot用于绘制柱状图，结果返回一个图形对象
    语法：graphic=barplot([locations,]values,[,buffer][,/current][,/device][,dimensions=array][,width=value][,bottom_values=value][,bottom_color=value][,fill_color=value]
    [,color=value][,outline=value][axis_style={0|1|2|3}][xrange=[min,max]][yrange=[min,max]] [,margin=array][,name=string][,title=string][,position=array][,/overplot]
    [,/nodata][window_title=string][,/horizontal])
    其中，参数locations用于设置各个柱体的位置，即横坐标，如果该参数没有设置的话默认为参数values各元素的下标值；参数values为绘图的数据；关键字buffer用于设置将图形保存在缓存中；关键字current用于设置在当前窗口内创建图形(重新绘制坐标轴)；
    关键字device用于设置关键字margin和position的值为设备坐标，如果该关键字未设置则默认为归一化坐标；关键字dimensions是一个两元素数组[width,height],用于设置窗口的宽度与高度，单位为像素；关键字width用于设置柱体的宽度，值为0~1,默认0.8；
    关键字bottom_values用于设置柱体的纵坐标起始值；关键字bottom_color用于设置柱体底部的色彩，如果该关键字设置则柱体色彩从bottom_color到fill_color渐变；关键字fill_color用于设置柱体色彩；
    ；关键字color用于设置柱体边框的颜色；关键字outline用于设置是否将各个柱体加上边框，0表示不加边框，默认值1表示加边框；关键字axis_style用于设置坐标轴的性质；关键字xrange和yrange用于设置x轴和y轴的范围；
    关键字margin用于设置图形四周空白的宽度，可以用一个值使四周空白区域的宽度相同，或者用一个4元素数组[left,bottom,right,top]分别设置四边空白区域宽度；关键字name用于设置图形对象的名称；关键字title用于设置图形的标题；
    关键字postion为一个4元素数组，用于设置图形在窗口内的位置；关键字overplot用于设置将当前图形叠加到现有图形上面，关键字nodata用于设置在窗口类只绘制坐标轴，不绘制曲线；关键字window_title用于设置图形窗口的标题；关键字horizontal用于设置将图形横过来，即绘制条形图。
    
    在柱状图的基础上，还可以通过errorplot函数添加误差线
    语法：graphic=errorplot([x,]y,yerror,[format][,/overplot][,linestyle=value][,errorbar_capsize=value][,errorbar_color=value]) 
    其中，参数x为图形横坐标的数据，如果该关键字未设置则默认为参数y的下标；参数y为图形纵坐标的数据；参数yerror为误差线的数据；参数format为一个字符串变量，用于设置曲线和符号的性质；关键字linestyle用于设置线型；关
    键字errorbar_capsize用于设置误差线端点横线的宽度，在0~1之间；关键字errorbar_color用于设置误差线的颜色。
4 直方图
    直方图通常以柱状图或者曲线图的方式来表达。如果用plot过程绘制曲线图，需要利用关键字psym将线型设置为直方图型(psym=10)
    也可以用bar_plot函数绘制直方图
